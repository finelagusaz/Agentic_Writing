...
4. **作者スポーン＋初稿執筆（Step 3）**: author を指示付きでスポーン → `current-draft.txt` 出力 → 読者をバックグラウンドスポーン（初稿のみ。前話の本文も参照ファイルとして渡す）
...
11. **エピソードレトロ（Step 7.6）**: KPT自動生成 → 設計者にTryを1件提案 → 承認されたら [TRY-NEXT] で次話に反映
12. **チームシャットダウン（Step 8）**
...
- **読者エージェントの滞留問題（対策済み）**: 読者は `run_in_background: true` でスポーンされるが、チームに所属するため idle 状態のまま tmux セッション内に残留する。**対策: Step 8 で読者全員にも `shutdown_request` を明示的に送信する**。送信エラー（既に終了済み）は無視する
