...
ディスカッションフェーズ（Step 4D）で以下のプロトコルに従う:
...
4. **ラウンド管理**: idle 通知のサイクルをラウンドとしてカウント
   - Step 4D: 最大2ラウンド
5. **打ち切り**: 最大ラウンド到達時、全メンバーに「ディスカッション終了。現在の判定で進めます」を SendMessage
6. **成果物確認**: ディスカッション後に改稿方針が変わった場合（4D→REVISION_NEEDED）、arc-review.md に追記指示し出力を検証
