...
- **キャラクター登場密度**: 今話の列を追加し、最も古い列（5話より前）を削除する。テーブルに存在しないキャラで、今話に登場したキャラまたは次話以降も追跡が必要なキャラがいれば行を追加する（既存列は `—` で初期化し、今話列のみ評価記号を記入）。各キャラの「薄い回連続」を再計算する。2話連続△以下のキャラがいれば「現在の警告」を更新する。**ただし `plot-outline.md` で初登場が後の話に計画されているキャラ（初登場前の `×` ）は警告対象外とする**
...
- **蓮の半透明パターン追跡**: 「蓮の半透明（主軸パターン）」行の今話列に、確定ドラフトで最も印象的に使用された半透明描写パターン（輪郭揺らぎ/透過/影欠落/他者認知/環境同化）を記録する。最も古い話の列を削除する。直前2話と同じパターンが主軸になっている場合は `要分散` を状態に記録する
- **シーン構造パターン更新**: 「シーン構造パターン（直近3話）」テーブルをスライドし、最も古い行を削除して今話の行を追加する。`主要シーン構造` は確定ドラフトの主なシーン展開を一行で要約する。`冒頭の入り方` は冒頭シーンの状況（移動中/静止/回想/対話等）を記録する。`食事場面` は有無と比重（メイン/サブ/なし）を記録する。「反パターン警告」を直近3話のパターンから再計算して更新する
...
- **主人公の能動性追跡**: 「主人公の能動性追跡（直近5話）」テーブルに今話の行を追加する。5話を超えた場合は最も古い行を削除する。`選択の型` は確定ドラフトから判断する（episode-brief の設計と実装が異なる場合は実装を記録）。`選ばなかった選択肢` は episode-brief の記載または確定ドラフトから読み取れる内容を記録する。反応のみの場合は `選ばなかった選択肢` を `—` とする
- **テンポプロファイル履歴**: 「テンポプロファイル履歴（直近5話）」テーブルに今話の行を追加する。5話を超えた場合は最も古い行を削除する。`プロファイル` は確定ドラフトのシーン密度・展開速度から判断する。`引きの型` は確定ドラフトの末尾から判断する（明示的謎/不安の種/感情の残響/選択の予感）
...
`story/plot-outline.md` を Read し、今話の記述と確定ドラフトの実際の展開を照合する:

1. **計画済みイベントの実行確認**: plot-outline の今話に予定されていたイベントが実行されたか
2. **前倒し消費の検出**: plot-outline の**後の話**に予定されていたキャラクター初登場・主要イベントが、今話で消費されていないか
3. 乖離があれば:
   - `story/handover-notes.md` に `[PLOT-DRIFT]` タグでスレッドを追加する（例: 「[PLOT-DRIFT] ブランの初登場が第7話予定→第3話で消費。第7話の転換イベント再設計が必要」）
   - ユーザーに警告を表示する
4. 乖離がなければ記録不要
...
- `story/handover-notes.md` に `[DESIGN-REVIEW]` タグで「第{幕番号}幕終了。`/arc-observe` でアーク観測を実施すること」スレッドを追加する
- ユーザーに以下を通知する：
...
全確定エピソードを通読し、設計アークと実際の展開のずれを検出・修正します。
次巻に進む場合は /setup-arc を実行してください。
```
