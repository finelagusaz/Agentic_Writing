...
- **伏線・要素の進展追跡**: 今話で進展した要素の「最終進展話」「最終進展の具体的内容」を更新し「未進展話数」を0にリセットする。進展しなかった要素の「未進展話数」を+1する。2話以上未進展の要素があれば「現在の警告」を更新する。**終盤モード時（`progress.md` の `is_endgame == true`）: 「微小進展」（言及のみ、状態変化なし）は未進展話数をリセットせず、警告に「実質未回収」と追記する。** 新たに追跡すべき要素があれば行を追加する
- **表現パターン累積**: 確定したエピソード本文を Grep で走査し、今話の「身体部位比喩（抽象）」と「ダッシュ（——）」の回数をカウントして更新する。カウントは**地の文とセリフを分離**して記録し、形式は `地の文回数(+セリフ回数)` とする（例: `3(+1)` は地の文3回・セリフ1回）。最も古い話のカウントを削除し5話合計を再計算する。**状態の判定には地の文カウントのみ使用する**（セリフカウントは参考値）。各行の「状態」を更新する（`1話上限` が数値なら「今話地の文回数 > 上限」で `要分散`、それ以外は `正常`。`1話上限` が `—` の行は「5話地の文合計 >= 10」で `要分散`、未満は `正常`）
...
- `story/premise.md`（想定話数 — DEFERRED先の検証用）
- `workspace/progress.md`（Endgame Info — deferred_overflow 確認用）

`story/handover-notes.md` を更新し、見出しの話番号を「第{番号+1}話より」に更新すること。**deferred_overflow に該当する項目がある場合は、想定話数内の話番号に再割り当てするか `[ACTIVE]` に昇格すること。** 完了報告を待つ。
...
4. **終盤到達チェック**（`progress.md` の `is_endgame == true` の場合のみ）: 残り全話の伏線回収スケジュールが plot-outline.md に反映されているか。不足時は追記する
