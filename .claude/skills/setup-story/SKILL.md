---
name: setup-story
description: 新しい作品の設定を対話的に構築する。ジャンル・テーマ・キャラクター・プロット・文体を7ステップで定義し、story/ に全資料を出力する。
---

# /setup-story — 作品セットアップスキル
...
| # | 制御項目 | ルール |
|---|---------|-------|
| 1 | **Show, don't tell** | 感情・状況・設定を直接説明せず行動・描写で伝える。説明セリフ禁止、感情直接描写禁止、メタ匂わせ禁止、要約的語り制限を含む |
| 2 | **文末バリエーション** | 同じ文末は連続2回まで。3回目は別パターン |
| 3 | **独自用語の創作禁止** | setting.md / characters.md にない固有名詞を使わない |
...
| # | 制御項目 | 選択肢 |
|---|---------|-------|
| 1 | ダッシュ（——）の使用 | 禁止 / 1話2回まで / 1話5回まで / 制限なし |
| 2 | 比喩・形容の量（直喩含む） | 1シーン1回 / 1シーン2回 / 制限なし |
| 3 | 体言止めの連続 | 2連続まで / 3連続まで / 制限なし |
| 4 | 修飾語の重ね | 厳しく制限 / 軽く注意 / 制限なし |
| 5 | 接続詞の多様性 | 同一接続詞の連続禁止 / 軽く注意 / 制限なし |
...
1. **基本文体** — 視点、トーン、文長、文末バリエーション、段落の長さ
2. **「見せる」原則（Show, Don't Tell）** — 以下の4カテゴリそれぞれに○/×の具体例を付けて記述:
   - 感情を見せる（身体反応・行動・五感で示す）
   - 世界観・設定を見せる（キャラの行動・会話の中で伝える）
   - 能力・強さを見せる（具体的描写で実感させる）
   - 関係性を見せる（やり取りや行動で浮かび上がらせる）
3. **記法ルール** — 改行・空行、会話文、心内語、シーン転換、固有名詞初出ルール
4. **描写ガイドライン** — 以下を含む:
   - 五感描写（毎シーン最低2感覚）
   ...
   - 比喩（作品主題に沿った比喩を優先、1シーンに凝った比喩は1〜2個）
   - 反復・対句（意図的な反復 vs 無意識な重複）
   - 伏線の埋め方（描写に自然に埋め込む、1話に新規伏線1〜2本）
5. **エピソード構成テンプレート** — 基本型に加え、以下のバリエーションを記載:
   - 冒頭クライマックス型 / 日常崩壊型 / 静謐一貫型 / 謎解き型 / 時間跳躍型 / 会話劇型
   - 構成の自己診断（直前2話と同じパターンを避ける）
6. **禁止事項** — メタナラティブ参照、現代語混入（ジャンル次第）、ご都合展開等
7. **表記の正式名称** — setting.md / characters.md を正とする旨
