---
name: setup-arc
description: 巻のシナリオアーク・キャラクターアークを設計する。三幕構成・感情頂点・役割マップ・キャラクター変容ステージを対話的に構築し、/write-episode の前提条件を整える。Use when user says "アーク設計", "setup arc", "巻の構成を作りたい", "シナリオアークを設計", or wants to plan story arc structure before writing. Do NOT use for episode writing (/write-episode), world setup (/setup-world), or story editing (/edit-story).
---
...
- Step G 実行時: `references/series-tracker-template.md` を Read する（series-tracker.md の初期テンプレート）
...
- [ ] 低強度（感情強度「低」）が2話連続していない（特に巻の序盤1〜3話。Web小説は序盤離脱率が最も高いため、序盤に低・低の連続配置を避ける。やむを得ない場合は備考に設計意図を明記）
- [ ] 巻の第1話にプロットの方向性を示すフック（三冊目の伏線、新たな脅威、目的地の提示など）が含まれている
...
5. `story/series-tracker.md` が存在しない場合、`references/series-tracker-template.md` を Read し、テンプレートの内容で新規作成する。
...
Phase 0 完了。アーク設計がロックされました。
  ...
  - story/series-tracker.md … 初期化済み（新規作成 or 既存維持）
...
| `story/series-tracker.md` | シリーズ横断追跡テンプレート（未存在時のみ新規作成） |
