...
| `arc-reviewer`（アークレビュアー） | 役割タグ準拠評価・改稿判定 | Sonnet | Phase 1 毎話 |
...
`PASS | REVISION_NEEDED | FORCE_PASS`
...
2. Step 0〜8 の順次実行
3. 条件付き分岐
   - `step4d`: REVISION_NEEDED 時のドラフトディスカッション
...
| ファイル | 生成/更新タイミング | 役割 |
| --- | --- | --- |
| `workspace/episode-brief.md` | Step 2 | エピソードブリーフ（アーク設計から導出・執筆アクセント含む） |
| `workspace/accent-note.md` | Step 3（変更時のみ） | 構成パターン/感覚レンズ変更メモ（Step 7 で転記後削除） |
| `workspace/current-draft.txt` | Step 3（改稿ごとに更新） | 執筆本文 |
| `workspace/arc-review.md` | Step 4 | アークレビュアーレビュー |
| `workspace/reader-feedback-*.md` | Step 3 後 bg 生成、Step 5 回収 | 読者 FB |
| `workspace/revision-log.md` | Step 0 初期化、Step 6 追記 | 改稿履歴 |
...
### 8.1 判定ロジック（Step 6）
1. arc-reviewer 判定と reader 評価（平均・中央値）を集計
2. `revision_count >= max_revisions` は `FORCE_PASS`
3. arc-reviewer が `PASS` かつ reader 平均が閾値以上で `PASS` もしくは `PASS_WITH_POLISH`
4. arc-reviewer が `REVISION_NEEDED` の場合は改稿ループへ
...
flowchart TD
    ...
    E --> F{"Step 6 判定"}
    F -->|PASS| G["Step 7-8\n確定・保存・終了"]
    F -->|PASS_WITH_POLISH| P["Step 6.5P\n軽微修正"]
    P --> G
    F -->|REVISION_NEEDED| R["Step 4D ディスカッション\n→ Step 3 へ戻る"]
    R --> E
    F -->|FORCE_PASS| G
    G -->|幕の境界| AO["/arc-observe\nアーク観測（Phase 2）"]
    AO --> W
```
